...
<%
    ...
    List<Vehicles> sublist = null;
    int r = 0;
    int tempint = 0;
    int size = 0;
    try {
        list = new VehicleService().getVehicles();
        if (request.getParameter("vehicletemp") != null && request.getParameter("vehicletemp").equals("1")) {
            ...
            list = new VehicleService().Search(vehicletemp);
        }
        size = list.size();
    } catch (Exception e) {
        e.printStackTrace();
...
<header class="bg-dark text-white p-3">
    <div class="container header-container">
        ...
        <div class="links">
            ...
            <a href="VehiclesAdd.jsp?Btn=Add Vehicle&temp=null" class="btn btn-secondary btn-sm" title="Add"><i class="fas fa-plus"></i></a>
...
<main class="container mt-4">
    <form action="Vehicle.jsp?Btn=Search" method="post">
        <div class="input-group mb-3">
            <input type="text" class="form-control" placeholder="Search by Reg.no" id="filter" name="filter">
...
<%
    ...
    request.setAttribute("vehicles", sublist);
%>
                ...
                <tbody>
                    <c:choose>
                        <c:when test="${empty vehicles}">
                            <tr>
                                <td colspan="8">No Vehicles are there in the DataBase yet.....</td>
                            </tr>
                        </c:when>
                        <c:otherwise>
                            <c:forEach items="${vehicles}" var="v">
                                <tr>
                                    ...
                                    <td>${v.type}</td>
                                    <td>${v.modelNo.name}</td>
                                    <td>${v.fuel}</td>
                                    <td>${v.capacity}</td>
                                    <td>${v.dailyRate}</td>
                                    <td class="actions">
                                        <a href="VehicleEdit.jsp?Btn=Edit&Number=${v.vehicleNo}" class="btn btn-outline-primary btn-sm" title="Modify"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="Vehicle.jsp?Btn=Delete&Number=${v.vehicleNo}" class="btn btn-outline-danger btn-sm" title="Delete"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </c:otherwise>
    ...
    <nav class="mt-4">
        <ul class="pagination justify-content-center">
            <li class="page-item">
                <a class="page-link" href="Vehicle.jsp?pageno=${currpage-1}">&laquo; Prev</a>
            </li>
            <c:forEach var="i" begin="${currpage}" end="${currpage+2}">
                <li class="page-item"><a class="page-link" href="Vehicle.jsp?pageno=${i}">${i}</a></li>
            </c:forEach>
            <li class="page-item"><a class="page-link" href="Vehicle.jsp?pageno=${currpage+1}">Next &raquo;</a></li>
        </ul>
    </nav>
</main>
...
<c:choose>
    <c:when test="${param.Btn=='Delete'}">
        <jsp:useBean id="vehicle" class="com.iss.models.Vehicles" scope="request">
            <jsp:setProperty name="vehicle" property="vehicleNo" value="${param.Number}" />
        </jsp:useBean>
        <%
            int Number = Integer.parseInt(request.getParameter("Number"));
            VehicleService vs = new VehicleService();
            vs.Delete(Number);
            response.sendRedirect("Vehicle.jsp?success=delete");
        %>
    </c:when>
    <c:when test="${param.Btn=='Search'}">
        <%
            ...
            response.sendRedirect("Vehicle.jsp?vehicletemp=1");
        %>
    </c:when>
</c:choose>
