...
<main class="container mt-4">
    ...
    <div class="form-container mt-4">
        <form action="Countryjsp/CountryAdd.jsp?btn=Add" method="post">
            <div class="form-group">
                <label for="country">Country number</label>
                <input type="text" class="form-control" id="countryNo" name="countryNo"  required>
...
<c:choose>
	<c:when test="${param.btn=='Add'}">
			<jsp:useBean id="tempcountry" class="com.iss.models.Countries" scope="request">
        		<jsp:setProperty name="tempcountry" property="country" value="${param.country}" />
        		<jsp:setProperty name="tempcountry" property="countryNo" value="${param.countryNo}"/>
			</jsp:useBean>
		    <%
		        CountriesService countryService = new CountriesService();
		        countryService.Add(tempcountry);
		        response.sendRedirect("Countryjsp/Countries.jsp?success=add");
		    %>	 
	</c:when>
</c:choose>
