...
<%
    ...
    // Retrieve owner details if editing an existing owner
    if (request.getParameter("ownerNo") != null) {
        ownerNo = Integer.parseInt(request.getParameter("ownerNo"));
        OwnerService ownerService = new OwnerService();
        Owners owner = ownerService.getOwner(ownerNo);

        if (owner != null) {
            ownerName = owner.getName();
            address1 = owner.getAddress1();
            address2 = owner.getAddress2();
            countryNo = owner.getCountryNo().getCountryNo();
            stateNo = owner.getStateNo().getStateNo();
            cityNo = owner.getCityNo().getCityNo();
            pincode = owner.getPincode();
            phoneNumber = owner.getPhoneNumber();
            mobileNumber = owner.getMobileNumber();
            bankName = owner.getBankName();
            accountNumber = owner.getBankAccount();
            panNumber = owner.getPAN();
        }
    }
%>
...
<header class="bg-dark text-white p-3">
    <div class="container header-container">
        ...
        <div class="links">
            ...
            <a href="Owner.jsp" class="btn btn-secondary btn-sm" title="Back"><i class="fas fa-arrow-left"></i></a>
...
<main class="container mt-4">
    ...
    <div class="form-container mt-4">
        <form action="OwnerEdit.jsp?btn=Edit" method="post">
            <fieldset>
                <legend>Owner Details</legend>
                <div class="form-group">
                    <label for="ownerNo">Owner Number (Don't Change!)</label>
                    <input type="text" class="form-control" id="ownerNo" name="ownerNo" value="<%= ownerNo %>" required readonly>
                </div>
                <div class="form-group">
